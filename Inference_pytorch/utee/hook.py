...
def hardware_evaluation(model, wl_weight, wl_activation, model_name, mode, cellBit, technode, reLu, memcelltype,
                        leveloutput, onoffratio, subArray):
    global model_n, FP
    model_n = model_name
    ...
    f.write('./NeuroSIM/main ./NeuroSIM/NetWork_' + str(model_name) + '.csv ' + str(wl_weight) + ' ' + str(
        wl_activation) + ' ' + str(cellBit) + ' ' + str(technode) + ' ' + str(reLu) + ' ' + str(memcelltype) + ' ' +
            str(leveloutput) + ' ' + str(onoffratio) + ' ' + str(subArray) + ' ')
